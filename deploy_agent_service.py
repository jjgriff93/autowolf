...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Deploy the Werewolf game to Azure AI Foundry Agent Service")

    parser.add_argument(
        "--subscription-id", type=str, help="Azure subscription ID (defaults to AZURE_SUBSCRIPTION_ID env var)"
    )

    parser.add_argument(
        "--resource-group", type=str, help="Azure resource group name (defaults to AZURE_RESOURCE_GROUP env var)"
    )
    ...
    parser.add_argument(
        "--model-deployment", type=str, help="Azure OpenAI model deployment name (defaults to MODEL_DEPLOYMENT env var)"
    )

    parser.add_argument("--model-name", type=str, help="Azure OpenAI model name (defaults to MODEL_NAME env var)")

    parser.add_argument(
        "--endpoint", type=str, help="Azure OpenAI endpoint URL (defaults to AZURE_OPENAI_ENDPOINT env var)"
    )

    parser.add_argument(
        "--api-version", type=str, default="2024-10-21", help="Azure OpenAI API version (default: 2024-10-21)"
    )
...
def create_werewolf_agent_functions() -> List[Function]:
    ...
    get_role_function = Function(
        ...
        parameters={
            "type": "object",
            "properties": {"player_id": {"type": "integer", "description": "The player ID to get the role for"}},
            "required": ["player_id"],
        },
    )
    ...
    cast_vote_function = Function(
        ...
        parameters={
            "type": "object",
            "properties": {
                "player_id": {"type": "integer", "description": "The ID of the player casting the vote"},
                "vote_for": {"type": "integer", "description": "The ID of the player being voted for"},
                "reason": {"type": "string", "description": "The reason for casting this vote"},
            },
    ...
    werewolf_action_function = Function(
        ...
        parameters={
            "type": "object",
            "properties": {
                "player_id": {"type": "integer", "description": "The ID of the werewolf player"},
                "target_id": {"type": "integer", "description": "The ID of the player to eliminate"},
            },
    ...
    seer_action_function = Function(
        ...
        parameters={
            "type": "object",
            "properties": {
                "player_id": {"type": "integer", "description": "The ID of the seer player"},
                "target_id": {"type": "integer", "description": "The ID of the player to check"},
            },
            "required": ["player_id", "target_id"],
        },
    )

    return [get_role_function, cast_vote_function, werewolf_action_function, seer_action_function]
...
def create_game_actions() -> List[Action]:
    ...
    start_game_action = Action(name="start_game", description="Start a new game of Werewolf")

    # End the current day phase and proceed to night
    end_day_phase_action = Action(name="end_day_phase", description="End the day phase discussion and move to voting")

    # End the current night phase and proceed to day
    end_night_phase_action = Action(name="end_night_phase", description="End the night phase and move to the next day")

    return [start_game_action, end_day_phase_action, end_night_phase_action]
...
def create_agent_definition(args) -> Dict[str, Any]:
    ...
    agent_definition = {
        ...
        "capabilities": {
            "functions": functions,
            "actions": {"definitions": actions, "overrides": ActionsOverrides(allow_additional_actions=False)},
        },
    }
...
def deploy_agent(args):
    ...
    ai_client.deployments.create_or_update(
        deployment_name=deployment_name, deployment=agent_definition, configuration=deployment_config
    )
